...
interface MultiSelectProps
	 ...
	 * Receives the current search string as an argument.
	 * Optional
	 */
	onSearchChange?: (search: string) => void;
	completeOptions?: MultiSelectOption[];
}
	...
	(
		...
		const getOptionByValue = React.useCallback(
			(value: string): MultiSelectOption | undefined => {
				if (completeOptions && onSearchChange) {
					return completeOptions.find((option) => option.value === value);
				}
				return getAllOptions().find((option) => option.value === value);
			},
			[getAllOptions, onSearchChange, completeOptions],
		);

		const filteredOptions = React.useMemo(() => {
			// If onSearchChange is provided, parent is handling filtering externally
			if (onSearchChange) {
				return options;
			}
			...
			<>
				...
				>
					...
					<PopoverContent
						...
						// Fix scroll propagation issue with Radix UI popovers
						// Without this, scroll events bubble up to parent elements (e.g., Dialog, page body)
						// causing the dropdown to appear stuck. See: https://github.com/radix-ui/primitives/issues/1159
